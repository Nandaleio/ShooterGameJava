...
import com.nandaleio.pixel.entity.Particles.GroundParticle;
...
public abstract class Mob extends Entity{
	...
	protected void jump(int jumpheight) {
		if(canjump) {
			...
			int sp = 0xffff00ff;
			if(!level.getTile(x, y).equals(Tile.voidtile)) {
				sp = level.getTile(x, y+1).getSprite().getGroundColor();
			}
				
			for(int i = 0 ; i < 10 ; i ++) {
				Particle p = new GroundParticle(x,y,50, sp);
				p.init(level);
				level.add(p);
			}
		}
