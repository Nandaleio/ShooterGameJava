...
public class Game extends Canvas implements Runnable {
	...
	public Game() {
		...
		//level.add(new ParticleSpawner(5, 1, 10, 40, 50));
		mouse = new Mouse();
		font = new Font();
	...
	public void render() {		
		...
		int limitX = level.getWidth()*16-width;
		int limitY =  level.getHeight()*16-height;
		
		int xScroll = level.getPlayer1().getX() - width/2;
		if(xScroll < 0) xScroll = 0;
		if(xScroll > limitX) xScroll = limitX;
		
		int yScroll = level.getPlayer1().getY() - height/2;
		if(yScroll < 0) yScroll = 0;
		if(yScroll > limitY) yScroll = limitY;
		...
		//font.render(5, 400, -1, "Nb entities : " + level.getEntitiesSize(), screen, 0xc7c7c7);
	...
	private int coPlusProche(int ancPix, int res) {
